...
internal class PrintMaxAVGRating : IQuery
{
    public SqlConnection connection { get; }
    public string table { get; }

    public PrintMaxAVGRating(SqlConnection connection, string table)
    {
    ...
    public async Task Query()
    {
        var command = new SqlCommand
        {
            ...
            CommandText = $"SELECT MAX([Середній бал]) AS [Максимальний з середніх], AVG([Максимальний бал]) AS [Середній з максимальних] FROM [{table}]" // текст команди
        };
