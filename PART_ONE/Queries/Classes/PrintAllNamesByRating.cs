...
// Виведення всіх [ПІБ] студентів з таблиці з мінімальною оцінкою, більше, ніж зазначена
internal class PrintAllNamesByRating : IQuery
{
    public SqlConnection connection { get; }
    public string table { get; }

    public PrintAllNamesByRating(SqlConnection connection, string table)
    {
    ...
    public async Task Query()
    {
        ...
        while (true)
        {
            ...
            if (int.TryParse(rating, out result)) break;
            else Console.WriteLine("Введено некоректне значення. Спробуйте ще...");
        }
        Console.WriteLine();
        ...
        var command = new SqlCommand
        {
            ...
            CommandText = $"SELECT [ПІБ],[Мінімальний бал] FROM [{table}] WHERE [Мінімальний бал] > {result}" // текст команди
        };
